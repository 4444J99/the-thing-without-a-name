...
git.
    ...
    ./4_corpus.py --limit 8            # isolated fast smoke build under .work/
"""
...
# WebP at these widths is ~25% under JPEG at matched visual quality. The shared
# dependency-free contract lets the renderer validate the same parameters.
TIERS = TIER_SPECS
...
SMOKE_MARKER = "generated by pipeline/4_corpus.py --limit\n"
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, help="output root (default: the public corpus)")
    ap.add_argument(
        "--limit",
        type=int,
        help="build only the first N frames in isolated .work output (smoke test)",
    )
    ...
    if args.limit is not None and args.limit < 1:
        print("--limit must be a positive frame count", file=sys.stderr)
        return 1
    explicit_out = args.out
    if args.limit is not None and explicit_out is not None:
        print("--limit owns isolated output under --work and cannot be combined with --out", file=sys.stderr)
        return 1
    ...
    if explicit_out is None:
        args.out = args.work / f"corpus-smoke-{args.limit}" if args.limit is not None else OUT
        if args.limit is not None:
            print(f"smoke output: {args.out}")
    else:
        args.out = explicit_out

    # A smoke run owns its marked isolated root. Recreate that generated root so
    # changing N or surviving an interrupted smoke cannot leave stale extras.
    # rmtree unlinks nested symlinks rather than following them; the marker
    # prevents this cleanup from claiming other data.
    if args.limit is not None:
        marker = args.out / ".danse-corpus-smoke"
        if args.out.exists() or args.out.is_symlink():
            try:
                marked = (
                    args.out.is_dir()
                    and not args.out.is_symlink()
                    and marker.is_file()
                    and not marker.is_symlink()
                    and marker.read_text() == SMOKE_MARKER
                )
            except (OSError, UnicodeError):
                marked = False
            if not marked:
                print(f"refusing unmarked smoke output: {args.out}", file=sys.stderr)
                return 1
            shutil.rmtree(args.out)
        args.out.mkdir(parents=True)
        marker.write_text(SMOKE_MARKER)
    ...
    def tier_is_current(name: str) -> bool:
        return tier_receipt_is_current(args.out, name, frame_ids, tier_identities[name])
    ...
    def tier_entry(name: str, nbytes: int) -> dict:
        return tier_manifest_entry(name, nbytes)
